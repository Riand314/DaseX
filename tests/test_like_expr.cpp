...
TEST(PlannerTest, LikeTest) {
	...
	std::string like_expr = "select * from lineitem where l_comment like '%pending%deposits%';";
	// 创建 binder，绑定 catalog

	Binder binder(global_catalog);
	auto start_parse = std::chrono::steady_clock::now();
	// 调用 pg 解析器进行词法语法解析
	binder.ParseAndSave(like_expr);
	...
	spdlog::info("[{} : {}] like解析时间: {}", __FILE__, __LINE__, Util::time_difference(start_parse, start_execute));
	spdlog::info("[{} : {}] like执行时间: {}", __FILE__, __LINE__, Util::time_difference(start_execute, end_execute));
	scheduler->shutdown();
