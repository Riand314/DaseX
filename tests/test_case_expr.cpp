...
TEST(PlannerTest, CaseTest) {
	...
	std::string case_expr = "SELECT l_extendedprice,\n"
		"       CASE WHEN l_comment like '%pending%deposits%' "
	...
	spdlog::info("[{} : {}] case解析时间: {}", __FILE__, __LINE__, Util::time_difference(start_parse, start_execute));
	spdlog::info("[{} : {}] case执行时间: {}", __FILE__, __LINE__, Util::time_difference(start_execute, end_execute));
	scheduler->shutdown();
